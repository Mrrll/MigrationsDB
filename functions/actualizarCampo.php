<?php
function actualizarCampo($base, $tabla, $campo, $pdo)
{
    ...
    if ($operador === 'BETWEEN') {        
        echo "> Introduce el valor de la condición WHERE ($campo $operador): ";
        $condicion_between = trim(fgets(STDIN));
        echo "> Introduce el valor de la condición BETWEEN ($campo AND): ";
        $condicion_and = trim(fgets(STDIN));
        $update_sql = "UPDATE $base.$tabla SET $campo = :nuevo_valor WHERE $campo BETWEEN :condicion_between AND :condicion_and";
    } elseif ($operador === 'IN') {
        echo "> Introduce los valores de la condición IN (separados por comas): ";
        $condicion_in = trim(fgets(STDIN));
        $condicion_in = "($condicion_in)";
        $update_sql = "UPDATE $base.$tabla SET $campo = :nuevo_valor WHERE $campo IN :condicion_in";
    } else {
        echo "> Introduce el valor de la condición WHERE ($campo $operador): ";
        $condicion_where = trim(fgets(STDIN));
        $update_sql = "UPDATE $base.$tabla SET $campo = :nuevo_valor WHERE $campo $operador :condicion_where";
    }

<?php

require_once 'functions/generarValoresFuentes.php';
require_once 'functions/resumenMapeos.php';
require_once 'functions/obtenerValores.php';
require_once 'functions/seleccionarTablaDestino.php';
require_once 'functions/generarOnDuplicateKeyUpdate.php';
require_once 'functions/generarSentenciaSql.php';
require_once 'functions/ejecutarSentenciasSql.php';
require_once 'functions/eliminarCampoAdicional.php';
require_once 'functions/obtenerTablasYCampos.php';
require_once 'functions/mostrarCampos.php';
require_once 'functions/obtenerEntradaValida.php';
require_once 'functions/detectarYConvertirFecha.php';
require_once 'functions/seleccionarTabla.php';
require_once 'functions/seleccionarCampo.php';
require_once 'functions/transformarCampo.php';
require_once 'functions/actualizarCampo.php';
require_once 'functions/generarSentenciaConNumerarFilasSecuencialmente.php';
require_once 'functions/crearCampoAdicional.php';
require_once 'functions/eliminarCampoAdicional.php';
require_once 'functions/condicionRelacion.php';
require_once 'functions/obtenerReferencia.php';


